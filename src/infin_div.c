...
void upd_rem_div(char *off, char **rem)
{
...
char *do_div(int *fin_nb_dig, int max_fin_nb_dig, char *take, char **rem)
{
    int i = 0;
...
char *start_div(char const *str1, char const *str2, int v1, int v2)
{
    int fin = my_strlen(skip(&str1[v1])) - (my_strlen(skip(&str2[v2])) - 1);
    int max_fin = fin;
    char *take;
    char *quot = malloc(sizeof(char) * fin + 2);
    ...
    for (; fin > 0; fin--) {
        if (my_strcmp(rem, "0") == 0)
            ...
            my_strcat(quot, do_div(&fin, max_fin, take, &rem));
        }
    }
    quot[max_fin + 1] = '\0';
    free(take);
    free(rem);
    return (clean_quot(quot, v1, v2));
}

char *infin_div2(char const *str1, char const *str2, int v1, int v2)
{
    ...
    if (my_strlen(skip(&str1[v1])) < my_strlen(skip(&str2[v2])) \
        || nb_zeros_const(&str1[v1]) == my_strlen(&str1[v1]))
        return ("0");
    return (start_div(str1, str2, v1, v2));
}
...
char *infin_div1(char const *str1, char const *str2)
{
    ...
    v1 = (str1[0] == '-') ? 1 : 0;
    v2 = (str2[0] == '-') ? 1 : 0;
    return (infin_div2(str1, str2, v1, v2));
}
